					...
					<tr>
						<td style="padding: 0 40px;">
							...
							<p style="margin: 0 0 24px; font-size: 14px; color: #666666; line-height: 1.6;">
								Cảm ơn bạn đã đăng ký tài khoản tại Lure Downloader. Vui lòng sử dụng mã PIN bên dưới để xác thực tài khoản của bạn:
							</p>
						</td>
					</tr>
					...
					<tr>
						<td style="padding: 24px 40px; background-color: #fafafa; border-radius: 0 0 16px 16px; border-top: 1px solid #eeeeee;">
							<p style="margin: 0; font-size: 12px; color: #999999; text-align: center;">
								© 2024 Lure Downloader. All rights reserved.
							</p>
						</td>
					</tr>
				</table>
...
export async function sendVerificationEmail(
	...
	try {
		await transporter.sendMail({
			from: `"Lure Downloader" <${process.env.SMTP_USER}>`,
			to: email,
			subject: "Xác thực tài khoản - Lure Downloader",
...
export async function sendPasswordResetEmail(
	...
	try {
		await transporter.sendMail({
			from: `"Lure Downloader" <${process.env.SMTP_USER}>`,
			to: email,
			subject: "Đặt lại mật khẩu - Lure Downloader",
