...
const pinStore = new Map<string, {pin: string; expiresAt: number; userData: {username: string; email: string; password: string}}>();
	...
	try {
		...
		if (action === "send-pin") {
			...
			if (!/^[a-zA-Z0-9_]{3,30}$/.test(username)) {
				return NextResponse.json(
					{success: false, error: "Username chỉ được chứa chữ cái, số và dấu gạch dưới (3-30 ký tự)"},
					{status: 400}
				);
			}
			...
			pinStore.set(email.toLowerCase(), {
				pin: newPin,
				expiresAt,
				userData: {username: username.toLowerCase(), email: email.toLowerCase(), password},
			});
		...
		if (action === "verify-pin") {
			...
			try {
				...
				if ((dbError as {code?: number}).code === 11000) {
					...
					return NextResponse.json(
						{success: false, error: "Email hoặc Username này đã được sử dụng"},
						{status: 400}
					);
				}
				...
				if ((dbError as {name?: string}).name === "ValidationError") {
					...
					return NextResponse.json(
						{success: false, error: validationError.message || "Dữ liệu không hợp lệ"},
						{status: 400}
					);
				}

				return NextResponse.json(
					{success: false, error: `Không thể tạo tài khoản: ${(dbError as Error).message}`},
					{status: 500}
				);
			}
		}
